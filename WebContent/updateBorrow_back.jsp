...
<%
			...
			String bno_tem = (String)session.getAttribute("bno");
			String bstate= request.getParameter("bstate");
			//判断是否一致
			if(bno.equals(bno_tem)){
				//判断当前状态
				if("未归还".equals(bstate)) {
					bstate = "归还";
					try {
						// 加载数据库驱动，注册到驱动管理器
						...
						String sql = "update BorrowI set bstate=?,bno=?  where bno='"+bno+"'";
						// 获取PreparedStatement
						PreparedStatement ps = conn.prepareStatement(sql);
						// 对SQL语句中的第1个参数赋值
						ps.setString(1,bstate);
						ps.setString(2,"1702010");
						// 执行更新操作，返回所影响的行数
